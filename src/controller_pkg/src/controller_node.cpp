...
: Node("controller_node", options)
{
  ...
  this->declare_parameter<double>("min_publish_interval_sec", 0.02);
  ...
  this->get_parameter("min_publish_interval_sec", config.min_publish_interval_sec);
  ...
  const robot_motion_msgs::msg::PlannedTrajectory::SharedPtr msg)
{
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "[task_id=%s] 收到 /planned_traj：point_count=%zu，timeout_sec=%.3f，准备启动轨迹执行",
    task_id.c_str(),
    msg->trajectory.points.size(),
    msg->timeout_sec);

  std::string error_msg;
  if (!executor_.start(task_id, msg->trajectory, this->now(), msg->timeout_sec, error_msg)) {
    RCLCPP_ERROR(
      this->get_logger(),
...
void ControllerNode::on_control_timer()
{
  ...
  if (result.finished) {
    RCLCPP_INFO(
      this->get_logger(),
      "[task_id=%s] 控制执行完成，final_error=%.6f",
      task_id.c_str(),
      result.current_error);
    ...
    publish_motion_event(
      ...
      result.message.empty() ? "控制层轨迹执行完成" : result.message,
      1.0F,
      result.current_error,
      false);
  ...
  double point_interval_sec)
{
  ...
  // 这里主要用于接口层执行，不再依赖 speed_scale / timeout_sec
  msg.speed_scale = 0.0;
  msg.timeout_sec = 0.0;

  // 关键优化点：
  // 将当前控制点的执行节拍继续传给接口层
  msg.point_interval_sec = point_interval_sec;
  msg.stamp = this->now();
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "[task_id=%s] 已发布 /joint_cmd：joint_count=%zu，point_interval_sec=%.4f",
    task_id.c_str(),
    joint_names.size(),
    point_interval_sec);
}
